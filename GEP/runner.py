...
def _log_model_debug(model: str) -> None:
    ...
    _p1_think = (
        _p1_dbg[0].get("thinking_mode", {}).get("supported", False) if _p1_dbg else "?"
    )
    _p2_think = (
        _p2_dbg[0].get("thinking_mode", {}).get("supported", False) if _p2_dbg else "?"
    )
...
def _parse_phase1(raw: str) -> dict | None:
    ...
    if re.search(r"\bFLAG\b", tu):
        issue_m = re.search(
            r"(?:issue|problem|error|found|type)[:\s]+([^\n.]+)", text, re.IGNORECASE
        )
        ...
        return {
            ...
            if issue_m
            else "Linguistic issue (prose response)",
            "quoted": quoted_m.group(1) if quoted_m else "",
            "confidence": 0.85,
        }
    if (
        re.search(r"\bCLEAN\b", tu)
        or re.search(r"\bOK\b", tu)
...
def _process_entry(
    ...
    ReaderReaction | None, str | None, str | None, float, dict | None, str | None
]:
    """
    ...
    _p1_rxn, p1_raw = call_ollama(
        PHASE1_MODEL, p1_system, p1_user, verbose=True, think=True, phase=1
    )
    ...
    phase1_result = (
        _parse_phase1(p1_raw or "") if (_p1_rxn is not None and p1_raw) else None
    )
...
def _print_reaction(
    ...
    if phase1_result:
        p1_icon = (
            "  🔤 FLAG" if phase1_result.get("verdict") == "FLAG" else "  ✅ CLEAN"
        )
    ...
    for i, entry in enumerate(pending, 1):
        ...
        if reaction is None:
            ...
            action_type = (
                ...
                if any(
                    x in (p2_raw or "")
                    for x in ["429", "HTTP", "timeout", "URLError", "OSError"]
                )
                else "error_parse"
            )
            record = build_record(
...
def run_overnight(
    ...
    try:
        for i, entry in enumerate(pending, 1):
            ...
            reaction, fragment_id, p2_raw, elapsed, phase1_result, p1_raw = (
                _process_entry(
                    entry, model, lang, version, year, genome, verbose=True, phase=phase
                )
            )
            ...
            if p1_raw:
                ...
                p1_clean = re.sub(
                    r"<think>.*?</think>", "", p1_raw, flags=re.DOTALL
                ).strip()
            ...
            think_info = (
                f"  thinking: {len(p2_think_text):,} chars\n" if p2_think_text else ""
            )
            ...
            p2_clean = re.sub(
                r"<think>.*?</think>", "", p2_raw or "", flags=re.DOTALL
            ).strip()
            if p2_clean:
            ...
            if entry_times:
                ...
                _log(
                    run_log,
                    f"  ETA: ~{hh2:02d}:{mm2:02d}:{ss2:02d} for {remaining} remaining  (avg {avg:.0f}s/entry)",
                )
