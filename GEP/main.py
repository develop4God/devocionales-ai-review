...
def _load_session() -> Session:
    try:
        if _SESSION_FILE.exists():
            ...
            return Session(
                **{k: v for k, v in raw.items() if k in Session.__dataclass_fields__}
            )
    except Exception:
        pass
    return Session()
...
def _numbered_menu(
    title: str, options: list[tuple[str, str]], back_label: str = "Back"
) -> str:
    """
...
def _stage_prepare(session: Session) -> Session:
    ...
    choice = _numbered_menu(
        "PREPARE  2/3  —  Bible Version", ver_opts, back_label="Back"
    )
    if choice == "0":
        return session
    version = choice
    ...
    if _confirm("Use a local source file?", default=bool(session.local_file)):
        _paths.ensure_dirs()
        local_file = _file_picker(
            "Select from data/source/", _paths.SOURCE_DIR, "*.json"
        )
        if not local_file:
            ...
            if seed_dir.exists():
                local_file = _file_picker(
                    "Select from seed_generation/yearly/", seed_dir, "*.json"
                )
    ...
    try:
        ...
        for p in _load_config().get("providers", []):
            if p.get("phase") == phase_key:
                opts.append(
                    (p["id"], f"{p.get('name', p['id'])}  ({p.get('model', '')})")
                )
        if opts:
            return opts
    except Exception:
...
def _stage_review(session: Session) -> None:
    ...
    if choice in ("flags", "all"):
        ...
        if _confirm("Pick specific input/results files?", default=False):
            _paths.ensure_dirs()
            input_file = _file_picker(
                "batch_input  file", _paths.BATCH_INPUT_DIR, "*.jsonl"
            )
            results_file = _file_picker(
                "batch_output file", _paths.BATCH_OUTPUT_DIR, "*.jsonl"
            )
...
def _show_genome_status(session: Session) -> None:
    ...
    print(
        _c(
            _box_top(f"GENOME  —  {session.lang} / {session.version} / {session.year}"),
            _CYAN,
            _BOLD,
        )
    )
    ...
    print(
        _c(
            _box_row(
                _c(
                    "  [coming soon]  Publish genome — promote fragments to production",
                    _DIM,
                )
            ),
            _DIM,
