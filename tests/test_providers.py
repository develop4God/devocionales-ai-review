...
def test_get_model_uses_default_provider_when_none_given():
    # default_provider is ollama_local (config/providers.yml) — a local provider
    # needs no API key, so this must succeed with no env setup at all.
    model = get_model()
