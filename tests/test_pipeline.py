...
try:
    ...
    check(
        "PauseCategory.GENERIC_REFLECTION resolves",
        cat == PauseCategory.GENERIC_REFLECTION,
    )
except ValueError:
    check(
        "PauseCategory.GENERIC_REFLECTION resolves",
        False,
        "ValueError — still missing from enum",
    )
...
check(
    "Genome has at least 1 fragment",
    len(genome.fragments) >= 1,
...
check(
    "Skip-reviewed logic works (5 dates in log)",
    len(reviewed) == 5,
    f"dates: {len(reviewed)}",
)
